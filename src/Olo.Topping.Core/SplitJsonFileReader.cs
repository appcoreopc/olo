...
namespace Olo
{
    public class SplitJsonFileReader : IJsonFileReader
    {
        public Task<string> GetJsonFileContent(string targetJsonFile)
        {
            var builders = new List<StringBuilder>();
            using (var reader = new StreamReader(targetJsonFile))
            {
                SplitJson(reader, "toppings", 2, i => { 
                    builders.Add(new StringBuilder()); return new StringWriter(builders.Last()); }, 
                    Formatting.Indented);
            }
